...
static int utf8prev(const char *s, int i) {
    if (i > 0)
...
static int utf8next(const char *s, int i) {
    if (s[i])
...
static int textwn(const char *s, int len) {
    XGlyphInfo ext;
...
static int textw(const char *s) { return textwn(s, (int)strlen(s)); }

/* draw s at x/baseline y, truncated to maxw px; a tab ends the visible
 * part of a line (the rest is the action, see execline) */
static void drawtext(int x, int y, XftColor *c, const char *s, int maxw) {
...
static void loadlist(const char *cmd) {
    FILE *f;
    char rb[4096];
    size_t n;
...
static void delete(int from, int to) {
...
static void xftcolor(const char *name, XftColor *c) {
...
static void setup(void) {
    ...
    win = XCreateWindow(
        dpy, root, x, y, (unsigned int)ww, (unsigned int)wh, borderw,
...
static int listwindows(void) {
    ...
    if (XGetWindowProperty(dpy, root, netclientlist, 0L, 1024L, False,
                           XA_WINDOW, &real, &fmt, &n, &extra, &p) != Success ||
        !p)
        return 0;
    list = (Window *)p;
    for (i = 0; i < n; i++) {
        title[0] = '\0';
        tp = NULL;
        if (XGetWindowProperty(dpy, list[i], netwmname, 0L, 64L, False, utf8,
                               &real, &fmt, &extra, &extra, &tp) == Success &&
            tp) {
...
static int activatewindow(const char *s) {
    XEvent ev;
...
static void desktopentry(const char *file) {
    FILE *f;
...
static void loadsource(const char *arg) {
    size_t i;
