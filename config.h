...
static const char *fontname   = "Iosevka NFM"; /* fontconfig name/pattern, HMENU_FONT */
static unsigned int fontsize  = 14;         /* px size appended to the pattern; 0 = pattern's own, HMENU_FONTSIZE */
static const char *col_bg     = "#1a1b26";  /* background, HMENU_BG */
static const char *col_fg     = "#c0caf5";  /* item text, HMENU_FG */
static const char *col_selbg  = "#7aa2f7";  /* selected item background, HMENU_SELBG */
static const char *col_selfg  = "#1a1b26";  /* selected item text, HMENU_SELFG */
static const char *col_prompt = "#7aa2f7";  /* input text and caret, HMENU_PROMPTCOL */
static const char *col_dim    = "#565f89";  /* match counter, separator, HMENU_DIM */
...
static const char *terminal   = "hterm";    /* Shift+Return runs `terminal -e sh -c line`, HMENU_TERMINAL */
static const char *fzfcmd     = "fzf";      /* filter program, run as `fzf --filter query` */
...
static const struct mode {
	...
	{ "run", "{ IFS=:; for d in $PATH; do [ -d \"$d\" ] && ls -1 \"$d\"; done; } 2>/dev/null | sort -u" },
	/* { "scripts", "ls -1 ~/bin" }, */
};
